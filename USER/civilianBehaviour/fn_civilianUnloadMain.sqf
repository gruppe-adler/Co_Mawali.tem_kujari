...
fnc_getSound = {
    ...
    private _prefix = if (isNull (_unit findNearestEnemy _unit)) then { "akin2" } else { "akin" };
    private _suffix = "01";

    switch (_type) do { 
        case "rice" : {
            private _suffix = (ceil (random 14)) call fnc_getSuffix;
        }; 
        case "adieu" : {
            private _suffix = (ceil (random 15)) call fnc_getSuffix;
        }; 
        case "water" : {
            private _suffix = (ceil (random 15)) call fnc_getSuffix;
        }; 
...

{
    ...
    if (speed _vehicle == 0 && _vehicle distance (leader group _x) < 500) then {
        ...
        for "_i" from 1 to (random 20 max 5) do {
            ...
            private _aboveSpawn = _spawnPosition;
            _aboveSpawn set [2, 100];
            if (lineIntersects [_spawnPosition, _aboveSpawn, objNull, objNull]) exitWith {}; // not inside
